...
const DEEP_CLOSER_MODULES = ['dashboard', 'employees', 'clients', 'equipment', 'salaries', 'expenses_professional', 'organization'];

export const DeepCloserApp = () => {
  const { can } = useAuth();
  ...
  useEffect(() => {
    if (!loading && enterprises.length > 0) {
      const enterprise = getEnterpriseBySlug('deep-closer');
      if (enterprise) {
  ...
  useEffect(() => {
    if (currentEnterprise) {
      const accessiblePages = availablePages.filter(page =>
        DEEP_CLOSER_MODULES.includes(page.module) && can(page.module, 'read', currentEnterprise.id)
      );
