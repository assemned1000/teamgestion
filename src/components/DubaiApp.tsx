...
const DUBAI_MODULES = ['dashboard', 'clients', 'expenses_professional'];

export const DubaiApp = () => {
  const { can } = useAuth();
  ...
  useEffect(() => {
    if (!loading && enterprises.length > 0) {
      const enterprise = getEnterpriseBySlug('dubai');
      if (enterprise) {
  ...
  useEffect(() => {
    if (currentEnterprise) {
      const accessiblePages = availablePages.filter(page =>
        DUBAI_MODULES.includes(page.module) && can(page.module, 'read', currentEnterprise.id)
      );
  ...
  const renderPage = () => {
    switch (currentPage) {
      case 'dashboard':
        return hasAccess('dashboard') ? <Dashboard /> : <div className="p-8 text-white">Accès refusé</div>;
      case 'employees':
